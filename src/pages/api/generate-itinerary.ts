...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['lantau-island', 'stanley', 'lamma-island'],
    'island': ['lamma-island', 'lantau-island', 'tai-o'],
    'hike': ['sai-kung', 'lantau-island', 'victoria-peak'],
    'hiking': ['sai-kung', 'lantau-island', 'victoria-peak'],
    'seafood': ['sai-kung', 'lamma-island', 'aberdeen'],
    'food': ['mong-kok', 'causeway-bay', 'wan-chai'],
    'dim sum': ['mong-kok', 'tsim-sha-tsui', 'central'],
    'shopping': ['mong-kok', 'causeway-bay', 'central'],
    'market': ['mong-kok', 'stanley', 'causeway-bay'],
    'culture': ['central', 'wan-chai', 'tai-o'],
    'history': ['central', 'stanley', 'wan-chai'],
    'night': ['mong-kok', 'tsim-sha-tsui', 'causeway-bay'],
    'family': ['lantau-island', 'tsim-sha-tsui', 'victoria-peak'],
    'honeymoon': ['victoria-peak', 'lamma-island', 'stanley'],
    'romantic': ['victoria-peak', 'lamma-island', 'stanley'],
    'budget': ['mong-kok', 'sai-kung', 'lamma-island'],
    'luxury': ['central', 'victoria-peak', 'tsim-sha-tsui'],
    'nature': ['sai-kung', 'lantau-island', 'lamma-island'],
    'adventure': ['sai-kung', 'lantau-island', 'victoria-peak'],
  };
  ...
  return ['victoria-peak', 'tsim-sha-tsui', 'central'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discovering Hong Kong AI Trip Planner. You generate detailed day-by-day itineraries for Hong Kong travel.

RULES:
- All prices in BOTH HKD and USD ($). Use rate: $1 = HKD 7.78
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-central"
- Transport: MTR (Octopus card), tram (HKD 3), Star Ferry (HKD 3.40-4.50), bus, taxi, kaito ferry
...
{
  "title": "string — trip title like '7-Day Hong Kong and Outlying Islands'",
  "subtitle": "string — budget + month like 'Mid-Range · October 2026'",
  "totalBudget": { "php": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrival and Victoria Peak'",
      "destination": "string — slug like 'victoria-peak'",
      "items": [
        {
          "time": "string — like 'Morning' or '5:30 PM'",
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Peak Tram Lower Terminus', 'Avenue of Stars', 'Sai Kung Waterfront'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Hong Kong trip:\n${parts.join('\n')}`;
